...
% Last Modified by GUIDE v2.5 17-Nov-2020 14:14:04
...
handles.stagePortNum = get(handles.stagePortList, 'Value');
...
if port >= 2
    if handles.connectedStage ~= true || handles.stagePortNum ~= port
        set(handles.connect2stage, 'enable', 'on');
...
port = get(hObject, 'Value');
if port >= 2
    if handles.connectedScope ~= true || handles.scopePortNum ~= port
...
handles.scopePortNum = get(handles.scopePortList, 'Value');
...
port = get(hObject, 'Value');
if port >= 2
    if handles.connectedLasers ~= true || handles.laserPortNum ~= port
...
set(handles.laserStatus, 'String', 'Connecting ...');
handles.laserPortNum = get(handles.laserPortList, 'Value');
...
handles.outputDirButton = uigetdir;
set(handles.outputDirText,'String',handles.outputDirButton);
guidata(hObject, handles);
end
...
% --- Executes on button press in concatSelected.
function concatSelected_Callback(hObject, eventdata, handles)
end
